...
output_dir=""
if [ "$1" == "--max_depth" ]; then
    [ "$#" -ne 4 ] && echo "использование: $0 [--max_depth N] входная_директория выходная_директория" && exit 1
    max_depth=$2
    input_dir=$3
    output_dir=$4
    preserve_structure=1
else
    [ "$#" -ne 2 ] && echo "использование: $0 [--max_depth N] входная_директория выходная_директория" && exit 1
    input_dir=$1
    output_dir=$2
fi
[ ! -d "$input_dir" ] && echo "ошибка: входная директория не существует" && exit 1
mkdir -p "$output_dir"
process_item() {
    local src=$1
    local dest=$2
